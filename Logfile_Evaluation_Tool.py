...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.messageFilter.addItems([
            ...
            "Belt trên cân bị tráo ID với belt trong laser"
        ])
        ...
        # Thêm giao diện cho tính năng tìm frame number theo timestamp
        self.timestampLabel = QLabel('Enter Timestamp (YY-MM-DD-HH:MM:SS):', self)
        self.timestampInput = QLineEdit(self)
        self.timestampInput.setPlaceholderText("e.g., 24-12-21-07:58:47")
        self.findFrameButton = QPushButton('Find Frame Number', self)
        self.findFrameButton.clicked.connect(self.find_frame_number)
        self.frameResultLabel = QLabel('Frame Number: N/A', self)
        ...
        buttonRowLayout.addWidget(self.exportButton)
        ...
        # Thêm layout cho tính năng tìm frame number
        timestampLayout = QHBoxLayout()
        timestampLayout.addWidget(self.timestampLabel)
        timestampLayout.addWidget(self.timestampInput)
        timestampLayout.addWidget(self.findFrameButton)
        timestampLayout.addWidget(self.frameResultLabel)
        topButtonLayout.addLayout(timestampLayout)
    ...
    def validate_timestamp_format(self, timestamp):
        """
        Kiểm tra định dạng timestamp có đúng YY-MM-DD-HH:MM:SS không.
        """
        ...
        # Tách các thành phần để kiểm tra giá trị hợp lệ
        try:
            parts = timestamp.split('-')
            year, month, day = parts[0], parts[1], parts[2].split(':')[0]
            hour, minute, second = parts[3].split(':')
    ...
    def find_frame_number(self):
        """
        Tìm frame number lớn nhất tương ứng với timestamp người dùng nhập.
        """
        try:
            # Lấy timestamp từ ô nhập liệu
            timestamp = self.timestampInput.text().strip()
            if not timestamp:
                self.frameResultLabel.setText("Frame Number: N/A (Please enter a timestamp)")
                self.frameResultLabel.setStyleSheet("color: red;")
                return

            # Kiểm tra định dạng timestamp
            if not self.validate_timestamp_format(timestamp):
                self.frameResultLabel.setText("Frame Number: N/A (Invalid format, use YY-MM-DD-HH:MM:SS)")
                self.frameResultLabel.setStyleSheet("color: red;")
                return

            # Kiểm tra xem DataFrame có dữ liệu không
            if self.df is None:
                self.frameResultLabel.setText("Frame Number: N/A (No data loaded)")
                self.frameResultLabel.setStyleSheet("color: red;")
                return

            # Tìm tất cả các hàng có timestamp khớp
            matching_frames = self.df[self.df['Timestamp'] == timestamp]

            if matching_frames.empty:
                self.frameResultLabel.setText(f"Frame Number: N/A (Timestamp '{timestamp}' not found)")
                self.frameResultLabel.setStyleSheet("color: red;")
                return

            # Lấy frame number lớn nhất (frame number là index của DataFrame)
            max_frame = matching_frames.index.max()

            # Hiển thị kết quả
            self.frameResultLabel.setText(f"Frame Number: {max_frame}")
            self.frameResultLabel.setStyleSheet("color: green;")

        except Exception as e:
            print(e)
            self.frameResultLabel.setText("Frame Number: N/A (Error occurred)")
            self.frameResultLabel.setStyleSheet("color: red;")
